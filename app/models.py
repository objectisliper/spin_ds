...
class StandardPerson:
    ...
    def __init__(self):
        ...
        self.__days_before_found_disease_avg = copy.deepcopy(get_setting('DISEASES_DETECT_LIST'))
    ...
    def live_a_day(self, person_to_connect, start_use_spin, new_year=False):
        if person_to_connect is not None:
            if not self.is_spin_user or person_to_connect.is_spin_user or decision(get_setting('SPIN_USER_CONNECT_SIMPLE_USER_LUCK')):
    ...
    def __try_to_heal(self, doctor=False):
        for disease_index, disease in enumerate(self.diseases):
            ...
            elif doctor and decision(get_setting('DISEASES_LUCK_HEAL_LIST')[disease]):
                self.diseases.pop(disease_index)
                if disease in self.known_diseases:
                    self.known_diseases.pop(self.known_diseases.index(disease))
            elif not doctor and decision(get_setting('DISEASES_DAILY_LUCK_HEAL_LIST')[disease]):
                self.diseases.pop(disease_index)
                if disease in self.known_diseases:
                    self.known_diseases.pop(self.known_diseases.index(disease))

    def __is_only_unhealable_known_diseases(self) -> bool:
        return all(disease in get_setting('UNHEALABLE_DISEASES') for disease in self.known_diseases)
